...
describe('invoice issue-on-invoice', () => {
  async function inventoryInvoice(quantity: number): Promise<string>{
  ...
  it('posts a separate COGS journal + outbound movement and preserves the issued cost', async () => {
    ...
    const id = await inventoryInvoice(10);
  ...
  it('blocks the whole issue when stock is insufficient (no revenue journal posted)', async () => {
    ...
    const before = journalCount();
    const id = await inventoryInvoice(50);
    const res = await useInvoiceStore.getState().issueInvoice(id);
...
describe('bill receive-on-bill', () => {
  it('records an inbound movement linked to the bill journal (no second journal)', async () => {
...
describe('supplier credit physical return', () => {
  it('records an outbound movement linked to the supplier-credit journal', async () => {
...
describe('documents without inventory lines', () => {
  it('a service invoice creates no stock movement', async () => {
    const { id } = await useInvoiceStore.getState().createDraft({ customerId: firstCustomerId() });
    const draft = useInvoiceStore.getState().getInvoice(id!)!;
    await useInvoiceStore.getState().updateDraft(id!, { lines: [{ ...draft.lines[0]!, accountId: acc('4120'), description: 'Consulting', quantity: 1, unitPrice: 500, taxRate: 0 }] });
...
describe('subledger reconciles to GL after document postings', () => {
  it('bill receipt + invoice issue keep the inventory subledger equal to GL', async () => {
    ...
    const sale = await useInvoiceStore.getState().createDraft({ customerId: firstCustomerId() });
    const sd = useInvoiceStore.getState().getInvoice(sale.id!)!;
    await useInvoiceStore.getState().updateDraft(sale.id!, { lines: [{ ...sd.lines[0]!, accountId: acc('4110'), quantity: 40, unitPrice: 20, taxRate: 0, inventoryItemId: 'item_goods', warehouseId: 'wh_main', inventoryFulfillmentMode: 'issue-on-invoice' }] });
    await useInvoiceStore.getState().issueInvoice(sale.id!);
