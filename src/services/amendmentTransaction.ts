...
/**
 * Run a sequence, rolling everything back if any step refuses or throws.
 *
 * The steps are closures rather than a data structure because each one needs
 * the ids the previous one produced. What this adds is the guarantee that a
 * `false` or an exception anywhere unwinds the whole thing — including a
 * failure inside a store the caller never mentioned, because the snapshot is
 * taken across all of them.
 */
export function runAtomically<T>(
  body: () => { ok: true; value: T } | { ok: false; error: string },
    ...
    return {
      ...
      error:
        cause instanceof Error
          ? `The amendment failed and everything was rolled back: ${cause.message}`
          : 'The amendment failed and everything was rolled back.',
    };
  }
}
