...
async function issuedInvoice(cid: string, issueDate: string, dueDate: string, unitPrice: number, qty = 1): Promise<string>{
  const { id } = await iStore().createDraft({ customerId: cid, issueDate, dueDate, currency: 'USD' });
  const inv = iStore().getInvoice(id!)!;
  await iStore().updateDraft(id!, { lines: [{ ...inv.lines[0]!, accountId: revenueId(), description: 'Svc', quantity: qty, unitPrice, taxRate: 0 }] });
...
async function receiptFor(cid: string, invoiceId: string, receiptDate: string, amount: number): Promise<string>{
...
describe('acceptance scenario (opening 2,000 → closing 5,000)', () => {
  async function setupAcceptance(): Promise<{ invId: string; cnNumber: string; receiptNumber: string }> {
    const cid = customerId();
    await issuedInvoice(cid, '2026-06-15', '2026-06-15', 2000); // prior → opening 2,000
    const invId = await issuedInvoice(cid, '2026-07-12', '2026-07-12', 8000);
    const cnId = creditNote(invId, '2026-07-12', 3000); // auto-applied
    const rId = await receiptFor(cid, invId, '2026-07-15', 2000);
  ...
  it('produces the expected opening, debits, credits, closing and reconciliation', async () => {
  ...
  it('running balances follow the ledger: 2,000 → 10,000 → 7,000 → 5,000', async () => {
    await setupAcceptance();
    const st = build();
  ...
  it('CSV export totals tie back to the statement closing balance', async () => {
    await setupAcceptance();
    const st = build();
...
describe('source-record inclusion & exclusion', () => {
  it('excludes draft and void invoices; includes issued invoices', async () => {
    const cid = customerId();
    await issuedInvoice(cid, '2026-07-05', '2026-07-05', 1000);
    await iStore().createDraft({ customerId: cid, issueDate: '2026-07-06', dueDate: '2026-07-06', currency: 'USD' }); // draft
    const voidId = await issuedInvoice(cid, '2026-07-07', '2026-07-07', 4000);
    await iStore().voidInvoice(voidId, 'error');
    const st = build();
    expect(st.periodDebits).toBe(1000); // only the issued, non-void invoice
    expect(st.lines.filter((l) => l.type === 'invoice')).toHaveLength(1);
  });

  it('excludes draft/void credit notes and reversed receipts', async () => {
    const cid = customerId();
    const invId = await issuedInvoice(cid, '2026-07-05', '2026-07-05', 5000);
    cnStore().createCreditNoteFromInvoice(invId); // draft CN — excluded
    creditNote(invId, '2026-07-06', 1000); // issued CN — included
    const rId = await receiptFor(cid, invId, '2026-07-08', 1000);
  ...
  it('filters by customer and entity', async () => {
    await issuedInvoice(customerId(), '2026-07-05', '2026-07-05', 1000);
    await issuedInvoice(otherCustomerId(), '2026-07-06', '2026-07-06', 9999);
    const st = build();
...
describe('double-counting protection', () => {
  it('does not show generated invoice/credit/receipt journals as extra events', async () => {
    const cid = customerId();
    const invId = await issuedInvoice(cid, '2026-07-05', '2026-07-05', 8000);
    creditNote(invId, '2026-07-06', 3000);
    await receiptFor(cid, invId, '2026-07-08', 2000);
    const st = build();
  ...
  it('includes an unlinked manual customer journal adjustment and stays reconciled', async () => {
    const cid = customerId();
    await issuedInvoice(cid, '2026-07-05', '2026-07-05', 1000);
...
describe('aging & outstanding schedule', () => {
  it('ages the remaining balance by due date into the correct buckets', async () => {
    const cid = customerId();
    await issuedInvoice(cid, '2026-06-15', '2026-06-15', 2000); // 46 days overdue at Jul 31 → 31-60
    const invId = await issuedInvoice(cid, '2026-07-12', '2026-07-12', 8000); // 19 days → 1-30
    creditNote(invId, '2026-07-12', 3000);
    await receiptFor(cid, invId, '2026-07-15', 2000); // INV-0006 outstanding now 3,000
    const st = build();
...
describe('statement types & edge cases', () => {
  it('balance-forward shows an opening balance; activity-only omits it', async () => {
    const cid = customerId();
    await issuedInvoice(cid, '2026-06-15', '2026-06-15', 2000);
    await issuedInvoice(cid, '2026-07-10', '2026-07-10', 1000);
  ...
  it('a net customer credit produces a negative closing balance', async () => {
    const cid = customerId();
    const invId = await issuedInvoice(cid, '2026-07-05', '2026-07-05', 1000);
    await receiptFor(cid, invId, '2026-07-06', 1000); // settles the invoice
  ...
  it('an empty customer renders a zero statement safely', async () => {
  ...
  it('base-currency mode converts using the stored exchange rate', async () => {
    const cid = customerId();
    const { id } = await iStore().createDraft({ customerId: cid, issueDate: '2026-07-05', dueDate: '2026-07-05', currency: 'EUR' });
    const inv = iStore().getInvoice(id!)!;
    await iStore().updateDraft(id!, { exchangeRate: 1.1, lines: [{ ...inv.lines[0]!, accountId: revenueId(), description: 'Svc', quantity: 1, unitPrice: 1000, taxRate: 0 }] });
    await iStore().issueInvoice(id!);
