...
/**
 * The fields this rule actually consults.
 *
 * Deliberately a `Pick` and not the whole record: eligibility depends on four
 * flags, and demanding a complete `AccountRecord` forced every caller to
 * hand-build one — so growing the record with a presentation field it never
 * reads would break call sites that are correct.
 */
export type PostingAccountFacts = Pick<AccountRecord, 'archived' | 'blocked' | 'active' | 'isPostable'>;

/** Server-authoritative universal eligibility rule for every new ordinary posting. */
export function assessPostingAccount(account: PostingAccountFacts, hasChildren = false): PostingAccountAssessment {
